...
## Latest milestone: text rendering working (Phase 4d core)

The affidavit text on the Sign-In screen now renders in legible English
in the correct position, color, and at full framerate. All six lines of
"The ClueFinders need YOU to help them rescue the professor..." appear
on the papyrus, centered at design x=260 to match the original RText
center-alignment behavior, drawn in dark brown from the font 50 NFNT
bitmap atlas.

Getting here was a multi-bug expedition. The atlas position metrics
generated by the extraction tool were off by one column because the tool
was incrementing the column counter when it skipped zero-width glyphs
(like space) but the actual atlas image generator did not - so every
character in the JSON was labeled with the column to its right, which
manifested as a Caesar cipher +1 shift in the rendered output ("Uif"
instead of "The"). Fixing the metrics tool to skip the col bump for
zero-width glyphs restored correct mapping.

Once letters rendered correctly, the text was pulsing in and out at half
framerate. The cause: every single glyph draw call was re-uploading the
234 KB font atlas to GS VRAM, so a single line of text triggered ~50 MB
of redundant texture transfers per frame and missed vsync. Adding an
"uploaded" flag on textures so VRAM upload happens once per texture
took FPS from 28 back to a locked 60 (NTSC half-rate 30 displayed) and
killed the flicker.

Two smaller bugs along the way: the MAX_FONTS slot table was sized 16
but the original game uses font IDs up to 50, so text_load_font was
silently returning -1 for font 50 with no debug output (because the
early bounds check ran before the printf). And the affidavit position
operand `x1=260` from the MPS bytecode is the center anchor for
RText, not the left edge - centering with `pen_x = 260 - text_width/2`
made the text fit inside the papyrus instead of running off the right.


## Previous milestone: full Sign-In screen with background

The complete Sign-In scene renders on PlayStation 2 with the original
...
### Phase 4d (text rendering) - core done, polish remaining

Done:
- NFNT bitmap font atlas generation (9 fonts: IDs 10, 11, 12, 20, 21,
  30, 31, 40, 50)
- Per-glyph metrics JSON pre-baked from NFNT data
- text_load_font / text_draw / text_measure / text_line_height in
  src/text.c with hand-rolled JSON parser (no malloc-heavy stdlib JSON)
- renderer_draw_glyph in src/renderer.c with per-glyph RGBAQ color
  modulation, alpha test, and shared atlas texture sampling
- One-shot atlas upload to GS VRAM via texture_t.uploaded flag
  (was the cause of the 28 FPS pulsing bug)
- Affidavit text displays correctly on Sign-In screen

Remaining:
- Wire RText engine objects to render actual scene-driven text instead
  of the hardcoded affidavit strings - promote OBJ_GENERIC RText to
  OBJ_TEXT with (string, color_id, x, y) fields, render from engine
  state in the main loop
- Color palette lookup: RText takes a color index (e.g. 10) referring
  to the SIGNIN RRGB palette - need to load the palette and resolve
  index to RGB at draw time (currently hardcoded dark brown)
- Render SignInList contents: placeholder "Player" text plus the
  list of saved names, with the textFontID/normalTextColor/
  selectedTextColor properties already being recorded by the engine
- Visual polish (deferred to Phase 7): font 50 atlas glyphs render
  about 1.6x larger than the original Mac game's drawn size because
  the NFNT bitmap is at native resolution but the original engine
  scaled to a smaller point size at draw time. Runtime GS scaling
  via dst_w/dst_h works but produced fuzzy edges; the right fix is
  to regenerate atlases pre-scaled with PIL Lanczos in a future pass.
